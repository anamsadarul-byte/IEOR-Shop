...
const ExecutiveSummary = () => {
  ...
  const kpis = [
    { title: "Total Orders", value: dashboardData.totalOrders.toLocaleString(), icon: Package, color: "text-primary" },
    { title: "Total Waste", value: dashboardData.totalWaste.toLocaleString(), icon: AlertTriangle, color: "text-destructive" },
    { title: "Unmet Demand", value: dashboardData.totalUnmetDemand.toLocaleString(), icon: BarChart3, color: "text-secondary" },
    { title: "Service Level", value: `${dashboardData.overallServiceLevel.toFixed(1)}%`, icon: TrendingUp, color: "text-accent" },
    { title: "Leftover Inventory", value: dashboardData.totalLeftover.toLocaleString(), icon: Archive, color: "text-muted-foreground" },
  ];
              ...
              <tbody>
                {dashboardData.items.map((item) => (
                  <tr key={item.item} className="border-b last:border-0 hover:bg-muted/50">
                    ...
                    <td className="text-right py-2 px-3">{item.totalOrdered}</td>
